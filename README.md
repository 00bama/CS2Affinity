...
### Firma de código (reducir advertencias de SmartScreen)

Firmar digitalmente el ejecutable reduce la probabilidad de que Windows/SmartScreen muestre advertencias. Recomendaciones:

- Usa un certificado Code Signing (preferiblemente EV).
- En GitHub, añade dos secretos en Settings → Secrets → Actions:
  - `SIGNING_PFX`: contenido del `.pfx` codificado en Base64.
  - `SIGNING_PFX_PASSWORD`: contraseña del `.pfx`.
- El workflow de release detectará los secretos y firmará `CS2Affinity.exe` antes de empaquetarlo en el ZIP.

Si prefieres firmar localmente, usa `signtool` y sube el ZIP firmado manualmente a la Release.
